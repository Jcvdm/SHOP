...
**The global `supabase` browser client carries Supabase auth context.** By having the audit service fall back to this global client when no explicit client is passed, all 18 services now automatically capture user information without individual refactoring. This is a clean, minimal change that provides maximum impact with zero service-level changes required.
